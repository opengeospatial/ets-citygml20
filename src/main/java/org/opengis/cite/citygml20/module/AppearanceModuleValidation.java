...
public class AppearanceModuleValidation extends CommonFixture {
	 ...
	 */
	@Test(enabled = true, description = "B.2.2 Appearance module") // groups = "B.2
																	// Conformance classes
																	// related to CityGML
																	// modules"
	 ...
	 * requirements. This test case is mandatory for all CityGML instance documents.
	 */
	@Test(enabled = true, description = "B.2.2 Appearance module - Clause 9.7 - Referential integrity 11",
			dependsOnMethods = { "verifyAppearanceModule" }) // groups = "B.2 Conformance
																// classes related to
																// CityGML modules"
	public void verifyAppearanceTextureCoordinatesToLinearRingId() throws Exception {
		...
		// Clause 9.7 - Referential integrity (11): The ring attribute of the
		// textureCoordinates property
		// of the element TexCoordList shall specify the gml:id of the target surface
		// geometry object which may
	...
	/**
	 * Clause 9.7. Base requirement 3. Each boundary point of the surface must receive a
	 * corresponding coordinate pair in texture space.
	 */
	@Test(enabled = true, description = "B.2.2 Appearance module - Clause 9.7. Base requirement 3",
			dependsOnMethods = { "verifyAppearanceModule" })
	public void verifyAppearanceTextureCoordinatesToLinearRingPosList() throws Exception {
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if (!n.hasAttributeNS("http://www.opengis.net/gml", "id")) {
				continue; // only check those linearings that are associated with
							// texturecoordinates
			}
			...
			String linearRingCoordinates = ((Element) n.getElementsByTagNameNS("http://www.opengis.net/gml", "posList")
			...
			String srsDimensionString = ((Element) n.getElementsByTagNameNS("http://www.opengis.net/gml", "posList")
				.item(0)).getAttribute("srsDimension");

			Assert.assertTrue((numberOfTextureCoordinates / 2) == (numberOfLinearRingCoordinates / Integer
				.parseInt(srsDimensionString)), "Problem with " + value
						+ ". Each boundary point of the surface must receive a corresponding coordinate pair in texture space.");
		}
